...
def _ComputeXSec_Data(data,method="default"):
    ...
    if method=="default":        
        if data.processType == "G2":
            XX=SnowFlake.G2List([d["<x>"] for d in data.points],
                                [d["<Q>"] for d in data.points],
                                [d["process"] for d in data.points])
            
        elif data.processType=="D2":
            XX=SnowFlake.D2List([d["<Q>"] for d in data.points],
                                [d["process"] for d in data.points])
    else:
...
def _ComputeXSec_Point(p,method="default"):
    ...
    if method=="default":        
        if p["type"] == "G2":
            XX1=SnowFlake.G2List([p["<x>"]],[p["<Q>"]],[p["process"]])
            
        elif p["type"]=="D2":
            XX1=SnowFlake.D2List([p["<Q>"]],[p["process"]])
        else:
