...
### Scale uncertanty
beamPolarizationUncertanty=0.014
...
print("Done.  =>     Create points & append to data set ...")
DataCurrent=DataProcessor.DataSet.DataSet('star26.sivers.W+.dy',"DY")
DataCurrent.comment="STAR AN for W+ (differential in y) TAKEN FROM THE PLOT"
DataCurrent.reference="????.????"

xxx=[-0.052,0.073,-0.106]
unun=[0.065,0.046,0.036]

proc_current=[1,1,1,10004]
proc_denominator=[1,1,1,4]
s_current=500.**2
includeCuts=False
cutParameters=[25.,25.,-1.,1.] #pT1,pT2,yMin,yMax
DataCurrent.normErr.append(beamPolarizationUncertanty)
...
for i in range(len(data_current)):
    ...
    p1["xSec"]=xxx[i]#data_current[i][3]
    p1["includeCuts"]=includeCuts
    p1["cutParams"]=cutParameters    
    p1["thFactor"]=1.         ### tobe updated
    p1["uncorrErr"].append(unun[i])
    #p1["uncorrErr"].append((data_current[i][4]-data_current[i][5])/2.)
    #p1["uncorrErr"].append((data_current[i][6]-data_current[i][7])/2.)
    p1["weightProcess"]=proc_denominator
    #
...
xxx=[-0.065,-0.020,0.044]
unun=[0.144,0.095,0.150]


proc_current=[1,1,1,10005]
proc_denominator=[1,1,1,5]
s_current=500.**2
includeCuts=False
cutParameters=[25.,25.,-1.,1.] #pT1,pT2,yMin,yMax
DataCurrent.normErr.append(beamPolarizationUncertanty)
...
print("Done.  =>     Create points & append to data set ...")
DataCurrent=DataProcessor.DataSet.DataSet('star23.sivers.Z',"DY")
DataCurrent.comment="STAR AN for Z"
DataCurrent.reference="2308.15496"

proc_current=[1,1,1,10003]
proc_denominator=[1,1,1,3]
s_current=500.**2
includeCuts=False
cutParameters=[25.,25.,-1.,1.] #pT1,pT2,yMin,yMax
DataCurrent.normErr.append(beamPolarizationUncertanty)
...
for i in range(1):
    ...
    p1["qT"]=[0.5,10.]    
    p1["<Q>"]=M_Z
    p1["Q"]=[73.,114.] ### I have checked that +- 30 gives ~10^{-3} error in comparison to the total volume
    p1["<y>"]=0.
    p1["y"]=[-1.,1.]    
    p1["xSec"]=0.056
    p1["includeCuts"]=includeCuts
    p1["cutParams"]=cutParameters    
    p1["thFactor"]=1.         ### tobe updated
    p1["uncorrErr"].append(0.081)
    p1["uncorrErr"].append(0.050)
    p1["weightProcess"]=proc_denominator
    #
    DataCurrent.AddPoint(p1)    
