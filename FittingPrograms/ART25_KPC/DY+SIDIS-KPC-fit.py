...
if RUN_CONFIGURATION=="LP" :
    path_to_constants=MAINPATH+"FittingPrograms/ART25_KPC/ConstantsFiles/ART_SIDIS_LP.atmde"
else:
    path_to_constants=MAINPATH+"FittingPrograms/ART25_KPC/ConstantsFiles/ART_SIDIS_KPC.atmde"
...
def cutFuncSIDIS(p):
    ...
    if RUN_CONFIGURATION=="KPC1":
        if p["process"][3] in [2001,2002,2103]:
            p["process"][3]+=40 #### these processes should be  20X. with X=4
        else:
            print("HEY HEY, something bad with you procesess")
            sys.exit()
...
setSIDIS=theData.CutData(cutFuncSIDIS) 
...
setA4=theDataA4.CutData(cutFuncDY) 

print('Loaded ', setA4.numberOfSets, 'data sets with', sum([i.numberOfPoints for i in setA4.sets]), 'points.')

### Load the data for A4UU
theDataA4UU=DataProcessor.DataMultiSet.DataMultiSet("DYset",loadThisDataDY([
    "A8_Auu_0y1","A8_Auu_1y2","A8_Auu_2y35",
    "CMS8-Auu-00y10","CMS8-Auu-10y21"]))
setA4UU=theDataA4UU.CutData(cutFuncDY) 
...
setDYcomplete=theData.CutData(cutFuncDY) 

print('Loaded ', setDYcomplete.numberOfSets, 'data sets with', sum([i.numberOfPoints for i in setDYcomplete.sets]), 'points.')
...
def chi2(x):
    ...
    YY=DataProcessor.harpyInterface.ComputeXSec(setDYcomplete)
    ccDY2,cc3=setDY.chi2(YY[0:setDY.numberOfPoints])
    
    A4=numpy.array(YY[setDY.numberOfPoints:setDY.numberOfPoints+setA4.numberOfPoints]
                    )/numpy.array(YY[setDY.numberOfPoints+setA4.numberOfPoints:setDY.numberOfPoints+2*setA4.numberOfPoints])
    ...
    cc=[ccSIDIS2/setSIDIS.numberOfPoints,ccDY2/setDY.numberOfPoints, ccA4/setA4.numberOfPoints,
        (ccSIDIS2+ccDY2+ccA4)/(setSIDIS.numberOfPoints+setDY.numberOfPoints+setA4.numberOfPoints)]
    
    endT=time.time()
    
    print(':->    ',"{:.4f}".format(cc[3]),"    = ("
          "{:.2f}".format((ccSIDIS2+ccDY2+ccA4)/(setSIDIS.numberOfPoints+setDY.numberOfPoints+setA4.numberOfPoints)),
          " + ","{:.2f}".format(ccSIDIS2/setSIDIS.numberOfPoints),
          " + ","{:.2f}".format(ccDY2/setDY.numberOfPoints),
          " + ","{:.2f}".format(ccA4/setA4.numberOfPoints),
          ")    time (DIS)=","{:.2f}".format(endTS-startT), 
          " time (DY)=","{:.2f}".format(endT-endTS)
          )
    
    return ccSIDIS2+ccDY2+ccA4
